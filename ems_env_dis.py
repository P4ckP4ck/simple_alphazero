...
class SimpleEnergyEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Diskrete Aktionslevels definieren
        self.battery_levels = np.linspace(0, self.battery_capacity, 6)  # z.B. [0, 1, ..., 5]
        self.heatpump_levels = np.linspace(0, self.heatpump_max_power, 6)  # z.B. [0, 0.166, ..., 0.83]
        self.actions = [(b, h) for b in self.battery_levels for h in self.heatpump_levels]
        self.action_space = spaces.Discrete(len(self.actions))  # z.B. 36 Aktionen
    ...
    def step(self, action):
        ...
        battery_discharge_requested, heatpump_input = self.actions[action]
